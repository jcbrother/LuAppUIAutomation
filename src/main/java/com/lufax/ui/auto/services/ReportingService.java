...
public class ReportingService {
    ...
    public String buildCaseDetailsDiv(Case aCase) throws IOException, DocumentException {
        ...
        String firstPicPath = getFirstElement(snapshootDataNodeStrList).getText();
        detailsDiv = detailsDiv.replace("$!{CASE_ID}","CASE-" + aCase.getId())
                ...
                .replace("$!{FIRST_SNAPSHOOT_PIC}",getFirstElement(snapshootDataNodeStrList).getText())
                .replace("$!{FIRST_PIC_IDX}",getFirstElement(snapshootDataNodeStrList).attribute("id").getValue())
    ...
    public String buildReport(String suiteName, Integer passNum, Integer failNum, Integer naNum ) throws IOException, DocumentException {
        ...
        reportTemplate.replace("$!{APP_VERSION}",propertiesCenter.getRunConfigs().get("package.version"))
    ...
    //生成报告到文件
    public void generateReport(String suiteName, Integer passNum, Integer failNum, Integer naNum) throws IOException, DocumentException {
